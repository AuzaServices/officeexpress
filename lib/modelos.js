...
// Se o renderizador HTML->PDF (Chromium) não estiver disponível ou falhar,
// faz fallback para o pdfkit, garantindo que o download nunca quebre.
async function gerarPDF(modeloId, dadosBrutos) {
  try {
    ...
    console.warn("⚠️ Chromium indisponível; usando fallback pdfkit para " + modeloId);
  } catch (err) {
    console.warn("⚠️ Falha no render HTML->PDF (" + modeloId + "):", err.message, "— usando fallback pdfkit");
  }
  return gerarPDFPdfkit(modeloId, dadosBrutos);
}

function gerarPDFPdfkit(modeloId, dadosBrutos) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fonteBold = FONTES_BOLD[estilo.fonte] || "Helvetica-Bold";
      ...
      // Foto (se houver) — recortada em círculo, igual à pré-visualização
      if (dados.foto) {
        try {
          ...
          if (fotoBuf.length) {
            ...
            const fy = doc.y + 10;
            doc.save();
            doc.circle(fx + ft / 2, fy + ft / 2, ft / 2).clip();
            doc.image(fotoBuf, fx, fy, { width: ft, height: ft });
            doc.restore();
            doc.y = fy + ft + 14;
          }
      ...
      doc.font(fonteBold).fontSize(estilo.tamanho + 12).fillColor(cor);
      ...
      const secao = (titulo) => {
        doc.moveDown(0.5);
        // Título em negrito (cor do modelo), igual à pré-visualização
        doc.font(fonteBold).fontSize(estilo.tamanho + 2).fillColor(cor);
        doc.text(titulo.toUpperCase());
        // Linha divisória fina e cinza claro, igual à pré-visualização
        doc.strokeColor("#e5e7eb").lineWidth(0.5).moveTo(50, doc.y + 2).lineTo(545, doc.y + 2).stroke();
        // Espaço entre a linha cinza e o conteúdo da seção
        doc.moveDown(0.45);
      };
