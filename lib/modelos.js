...
async function gerarDOCX(modeloId, dadosBrutos) {
  ...
  try {
    ...
    img = null;
    console.error("❌ Chromium falhou ao renderizar o modelo (caindo para DOCX sem modelo):", e && e.message);
  }
  if (img && img.length) {
  ...
  console.warn("⚠️ DOCX gerado SEM modelo (fallback texto) — Chromium não produziu imagem para o modelo:", modeloId);
