...
async function gerarCartaDOCX(modeloId, dados) {
  const carta = montarCarta(modeloId, dados);
  const estilo = carta.modelo.estilo;
  const fonte = estilo.fonte === "serif" ? "Georgia" : "Aptos";
  const alinhamentoTitulo = ["faixa", "bloco"].includes(estilo.layout) ? AlignmentType.CENTER : AlignmentType.LEFT;
  const run = (text, extra = {}) => new TextRun({ text, font: fonte, size: 22, ...extra });
  const children = [
    new Paragraph({ alignment: alinhamentoTitulo, spacing: { after: 70 }, children: [run(carta.nome, { bold: true, size: 34, color: estilo.cor.replace("#", "") })] }),
    new Paragraph({ alignment: alinhamentoTitulo, spacing: { after: 420 }, children: [run(carta.contato.join("  |  "), { size: 18, color: "64748B" })] }),
    new Paragraph({ spacing: { after: 100 }, children: [run(carta.data, { size: 20, color: "64748B" })] }),
    new Paragraph({ spacing: { after: 310 }, children: [run(carta.assunto.toUpperCase(), { bold: true, size: 18, color: estilo.cor.replace("#", "") })] }),
    new Paragraph({ spacing: { after: 220 }, children: [run(carta.saudacao)] }),
    ...carta.paragrafos.map((paragrafo) => new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 220 }, children: [run(paragrafo.texto)] })),
    new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 400 }, children: [run(carta.encerramento)] }),
    new Paragraph({ spacing: { after: 320 }, children: [run(carta.despedida)] }),
    new Paragraph({ children: [run(carta.assinatura, { bold: true, color: estilo.cor.replace("#", "") })] }),
  ];
  return Packer.toBuffer(new Document({ sections: [{ properties: {}, children }] }));
}

module.exports = { CARTAS, montarCarta, montarTexto, gerarCartaPDF, gerarCartaDOCX };
