...
// Gera o HTML da prévia da carta aplicando o LAYOUT VISUAL do modelo escolhido,
// de forma consistente com o PDF gerado pelo servidor. Retorna o conteúdo interno
// do "papel" da carta (com estilos inline), pronto para injetar na prévia.
function montarCartaHTML(modeloId, dadosBrutos = {}) {
  const carta = montarCarta(modeloId, dadosBrutos);
  const { estilo } = carta.modelo;
  const cor = estilo.cor;
  const corSuave = estilo.corSuave;
  const sans = estilo.fonte === "serif" ? false : true;
  const familia = sans ? "Inter, Arial, sans-serif" : "Georgia, 'Times New Roman', serif";
  const contato = carta.contato.map(escaparHTML).join("  |  ");
  const nome = escaparHTML(carta.nome);

  let cabecalho = "";
  if (estilo.layout === "faixa") {
    cabecalho =
      `<div style="background:${cor};color:#fff;padding:34px 40px;border-radius:8px;margin-bottom:26px">` +
      `<div style="font-size:26px;font-weight:800;letter-spacing:.3px">${nome}</div>` +
      `<div style="font-size:11px;margin-top:6px;color:rgba(255,255,255,.85)">${contato}</div>` +
      `</div>`;
  } else if (estilo.layout === "lateral") {
    cabecalho =
      `<div style="display:flex;gap:22px;margin-bottom:26px">` +
      `<div style="width:5px;border-radius:3px;background:${cor};flex-shrink:0"></div>` +
      `<div>` +
      `<div style="font-size:25px;font-weight:800;color:${cor}">${nome}</div>` +
      `<div style="font-size:11px;margin-top:6px;color:#64748b">${contato}</div>` +
      `</div></div>`;
  } else if (estilo.layout === "bloco") {
    cabecalho =
      `<div style="background:${corSuave};border-radius:10px;padding:28px 30px;margin-bottom:26px">` +
      `<div style="font-size:24px;font-weight:800;color:${cor}">${nome}</div>` +
      `<div style="font-size:11px;margin-top:6px;color:#475569">${contato}</div>` +
      `</div>`;
  } else {
    // classico / linha
    cabecalho =
      `<div style="border-bottom:3px solid ${cor};padding-bottom:14px;margin-bottom:12px">` +
      `<div style="font-size:25px;font-weight:800;color:${cor}">${nome}</div>` +
      `<div style="font-size:11px;margin-top:6px;color:#64748b">${contato}</div>` +
      `</div>`;
  }

  const paragrafos = carta.paragrafos
    .map((p) => `<p style="font:15px/1.85 ${familia};margin:0 0 18px;color:#1f2937;text-align:justify">${escaparHTML(p.texto)}</p>`)
    .join("");

  return (
    cabecalho +
    `<div style="font-size:11px;color:#64748b;margin-bottom:22px">${escaparHTML(carta.data)}</div>` +
    `<div style="font-weight:800;font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:${cor};margin-bottom:18px">${escaparHTML(carta.assunto)}</div>` +
    `<p style="font:15px/1.85 ${familia};margin:0 0 18px;color:#1f2937">${escaparHTML(carta.saudacao)}</p>` +
    paragrafos +
    `<p style="font:15px/1.85 ${familia};margin:0 0 18px;color:#1f2937;text-align:justify">${escaparHTML(carta.encerramento)}</p>` +
    `<div style="font:15px/1.6 ${familia};color:#1f2937;margin-top:8px">${escaparHTML(carta.despedida)}</div>` +
    `<div style="font:700 15px/1.6 ${familia};color:${cor};margin-top:6px">${nome}</div>`
  );
}

module.exports = { CARTAS, montarCarta, montarTexto, gerarCartaPDF, montarCartaHTML };
