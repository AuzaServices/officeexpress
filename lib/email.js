...
async function enviarConfirmacao(email, nome, token) {
  ...
  `, "Confirme seu e-mail");
  return enviarEmail({ to: email, subject: "Confirme seu e-mail - Office Express", html: conteudo });
}
...
async function enviarRecuperacao(email, nome, token) {
  ...
  `, "Redefinir senha");
  return enviarEmail({ to: email, subject: "Redefinir sua senha - Office Express", html: conteudo });
}
