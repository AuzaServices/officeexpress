...
async function garantirSchema() {
  try {
    ...
    `);
    // Acumulação: a chave única (parceiro, mes) impedia que um parceiro que
    // já recebeu o repasse do mês acumulasse comissões novas do mesmo mês.
    // Substituída por um índice normal — permite uma linha 'pago' (histórico)
    // e uma linha 'apagar' (em acumulação) por parceiro/mês.
    try {
      await pool.query("ALTER TABLE pagamentos_parceiros DROP INDEX uniq_pag_parceiro_mes");
      console.log("🔧 pagamentos_parceiros: chave única (parceiro,mes) removida para permitir acumulação.");
    } catch (e) { /* índice já removido */ }
    try {
      await pool.query("CREATE INDEX idx_pag_parceiro_mes ON pagamentos_parceiros (parceiro_id, mes_ref)");
    } catch (e) { /* índice já existe */ }

    // =====================================================================
    // PLANOS DO CLIENTE (assinaturas): colunas em usuarios + tabelas de
    // pagamento e uso mensal. Idempotente — seguro a cada boot.
    // =====================================================================
    for (const col of [
      ["plano", "ENUM('gratuito','premium','premium_plus') NOT NULL DEFAULT 'gratuito'"],
      ["assinatura_ativa", "TINYINT(1) NOT NULL DEFAULT 0"],
      ["assinatura_expira_em", "DATETIME NULL"],
      ["mp_assinatura_id", "VARCHAR(80) NULL"],
    ]) {
      try {
        const [c] = await pool.query(
          "SELECT COUNT(*) AS n FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'usuarios' AND COLUMN_NAME = ?",
          [col[0]]
        );
        if (Number(c[0].n) === 0) {
          await pool.query(`ALTER TABLE usuarios ADD COLUMN ${col[0]} ${col[1]}`);
          console.log(`✅ Coluna usuarios.${col[0]} adicionada`);
        }
      } catch (e) {
        if (e.code !== "ER_DUP_ENTRY") console.error(`⚠️ Migração usuarios.${col[0]}:`, e.message);
      }
    }

    await pool.query(`
      CREATE TABLE IF NOT EXISTS usuarios_pagamentos (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        tipo ENUM('pix','card','assinatura') NULL,
        periodo_ref VARCHAR(20) NULL,
        pedido_pendente_id INT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        pago_at DATETIME NULL,
        INDEX idx_upag_usuario (usuario_id),
        INDEX idx_upag_pagamento (pagamento_id)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    `);

    // Migração idempotente: coluna pedido_pendente_id (currículo pendente que
    // o usuário escolheu incluir na assinatura na página de pagamento).
    try {
      const [c] = await pool.query(
        "SELECT COUNT(*) AS n FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'usuarios_pagamentos' AND COLUMN_NAME = 'pedido_pendente_id'"
      );
      if (Number(c[0].n) === 0) {
        await pool.query("ALTER TABLE usuarios_pagamentos ADD COLUMN pedido_pendente_id INT NULL");
        console.log("✅ Coluna usuarios_pagamentos.pedido_pendente_id adicionada");
      }
    } catch (e) {
      if (e.code !== "ER_DUP_ENTRY") console.error("⚠️ Migração usuarios_pagamentos.pedido_pendente_id:", e.message);
    }

    // Migração idempotente: coluna parceiro_id (parceiro indicador do usuário
    // que assinou — para o admin ver a origem do pagamento no painel e a
    // comissão de assinatura sair igual à do currículo avulso).
    try {
      const [cp] = await pool.query(
        "SELECT COUNT(*) AS n FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'usuarios_pagamentos' AND COLUMN_NAME = 'parceiro_id'"
      );
      if (Number(cp[0].n) === 0) {
        await pool.query(
          "ALTER TABLE usuarios_pagamentos ADD COLUMN parceiro_id INT NULL, ADD INDEX idx_upag_parceiro (parceiro_id)"
        );
        console.log("✅ Coluna usuarios_pagamentos.parceiro_id adicionada");
      }
    } catch (e) {
      if (e.code !== "ER_DUP_ENTRY") console.error("⚠️ Migração usuarios_pagamentos.parceiro_id:", e.message);
    }

    await pool.query(`
      CREATE TABLE IF NOT EXISTS usuarios_uso (
        usuario_id INT NOT NULL,
        periodo_ref VARCHAR(7) NOT NULL,
        curriculos_criados INT NOT NULL DEFAULT 0,
        PRIMARY KEY (usuario_id, periodo_ref)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    `);
    ...
    // Usa ON DUPLICATE KEY UPDATE (e não INSERT IGNORE): se a transação
    // existe mas ficou com parceiro/comissão NULL (pedido criado antes do
    // vínculo chegar), ela é CORRIGIDA em vez de descartada em silêncio.
    try {
      const [pagas] = await pool.query(
        `SELECT p.id, p.usuario_id, p.valor, p.parceiro_id, p.modelo, p.pagamento_tipo, p.pago_at,
                COALESCE(p.parceiro_id, u.parceiro_id) AS parceiro_efetivo
         FROM pedidos p
         LEFT JOIN usuarios u ON u.id = p.usuario_id
         WHERE p.status = 'pago'`
      );
      for (const v of pagas) {
        const pid = v.parceiro_efetivo || null;
        // Persiste o vínculo recuperado no pedido (consistência).
        if (pid && !v.parceiro_id) {
          await pool.query("UPDATE pedidos SET parceiro_id = ? WHERE id = ?", [pid, v.id]);
        }
        await pool.query(
          `INSERT INTO transacoes (pedido_id, usuario_id, parceiro_id, modelo, valor, comissao_pct, tipo, pagamento_tipo, created_at)
          ...
          [v.id, v.usuario_id, pid, v.modelo, v.valor, pid, v.pagamento_tipo || "pix", v.pago_at || new Date()]
        );
      }
    } catch (err) {
    ...
    // Migração crítica: a coluna `tipo` de email_tokens é um ENUM antigo que
    // só aceita 'confirmacao' e 'recuperacao'. Sem incluir 'convite', o MySQL
    // grava string vazia e o convite nunca é encontrado (dava "convite
    // inválido, expirado ou já utilizado"). Idempotente e tolerante a falha.
    try {
      const [tipoCol] = await pool.query(
        "SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'email_tokens' AND COLUMN_NAME = 'tipo'"
      );
      const tipoAtual = tipoCol.length ? String(tipoCol[0].COLUMN_TYPE) : "";
      if (tipoAtual && tipoAtual.indexOf("convite") === -1) {
        await pool.query(
          "ALTER TABLE email_tokens MODIFY COLUMN tipo ENUM('confirmacao','recuperacao','convite') NOT NULL"
        );
        console.log("✅ Coluna email_tokens.tipo agora aceita 'convite'");
        // Repara convites gravados com tipo vazio pelo bug anterior.
        try {
          const [reparo] = await pool.query(
            "UPDATE email_tokens SET tipo = 'convite' WHERE tipo = '' AND payload_json LIKE '%parceiro_id%'"
          );
          if (reparo.affectedRows > 0) console.log(`🛠️ ${reparo.affectedRows} convite(s) com tipo vazio reparados`);
        } catch (e) {}
      }
    } catch (e) {
      console.error("🚨 FALHA ao atualizar email_tokens.tipo para aceitar 'convite':", e.message);
      console.error("🚨 Execute manualmente no phpMyAdmin: ALTER TABLE email_tokens MODIFY COLUMN tipo ENUM('confirmacao','recuperacao','convite') NOT NULL;");
    }
...
// Preço customizado POR PARCEIRO (pai ou filho). NULL = usa o preço padrão do sistema.
async function garantirPrecoParceiro() {
  try {
    const [cols] = await pool.query(
      "SELECT COUNT(*) AS c FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'parceiros' AND COLUMN_NAME = 'preco_custom'"
    );
    if (Number(cols[0].c) === 0) {
      await pool.query("ALTER TABLE parceiros ADD COLUMN preco_custom DECIMAL(10,2) NULL DEFAULT NULL");
      console.log("✅ Coluna parceiros.preco_custom adicionada");
    }
  } catch (e) {
    console.error("🚨 FALHA ao garantir parceiros.preco_custom:", e.message);
  }
}

// Resolve o preço do currículo para um dado parceiro (se houver custom) —
// usado tanto no checkout (via ref/link) quanto na exibição pública.
async function getPrecoParceiro(parceiroId) {
  if (!parceiroId) return null;
  try {
    const [rows] = await pool.query("SELECT preco_custom FROM parceiros WHERE id = ? AND ativo = 1", [parceiroId]);
    if (rows.length && rows[0].preco_custom != null) {
      const v = parseFloat(rows[0].preco_custom);
      if (!isNaN(v) && v > 0) return v;
    }
  } catch (e) {
    // coluna ainda não existe ou erro — cai no preço padrão
  }
  return null;
}

module.exports = { pool, garantirSchema, getPreco, setPreco, garantirPrecoParceiro, getPrecoParceiro };
