...
async function garantirSchema() {
  try {
    ...
    // ------------------------------------------------------------------
    await garantirColuna("parceiros", "pai_id", "pai_id INT NULL");
    await garantirColuna("parceiros", "tipo", "tipo VARCHAR(10) NOT NULL DEFAULT 'independente'");
    await garantirColuna("parceiros", "promovido_em", "promovido_em DATETIME NULL");
    await garantirColuna("transacoes", "bonus_pai_id", "bonus_pai_id INT NULL");
    await garantirColuna("transacoes", "bonus_pct", "bonus_pct DECIMAL(5,2) NULL");
    try {
      await pool.query("ALTER TABLE transacoes ADD INDEX idx_trans_bonus_pai (bonus_pai_id)");
    } catch (e) { /* índice já existe */ }

    // Backfill: parceiros sem tipo ficam 'independente'; os que já têm pai
    // ficam 'filho'.
    try {
      await pool.query("UPDATE parceiros SET tipo = 'independente' WHERE tipo IS NULL OR tipo = ''");
      await pool.query("UPDATE parceiros SET tipo = 'filho' WHERE pai_id IS NOT NULL AND tipo = 'independente'");
    } catch (e) {}
