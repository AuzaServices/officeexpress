...
// 🧬 Migração defensiva: garante as colunas necessárias para download por 24h
async function garantirSchemaPdfs() {
  try {
    const [cols] = await pool.query(
      "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'pdfs'"
    );
    const existentes = new Set(cols.map((c) => c.COLUMN_NAME));

    if (!existentes.has("token")) {
      await pool.query(
        "ALTER TABLE pdfs ADD COLUMN token VARCHAR(64) NULL, ADD UNIQUE INDEX uniq_pdf_token (token)"
      );
      console.log("✅ Coluna 'token' adicionada em pdfs");
    }
    if (!existentes.has("pago_at")) {
      await pool.query("ALTER TABLE pdfs ADD COLUMN pago_at DATETIME NULL");
      console.log("✅ Coluna 'pago_at' adicionada em pdfs");
    }
  } catch (err) {
    console.error("⚠️ Não foi possível garantir schema de pdfs:", err.message);
  }
}
garantirSchemaPdfs();

// Gera um token único para identificação do currículo pago
function gerarToken() {
  return (
